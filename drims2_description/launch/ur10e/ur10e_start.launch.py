...
def generate_launch_description():
  launch_args = [
    ...
    DeclareLaunchArgument(name="checkerboar_x",         default_value="-0.673",     description="Checkerboar x"),
    DeclareLaunchArgument(name="checkerboar_y",         default_value="0.353",      description="Checkerboar y"),
    DeclareLaunchArgument(name="checkerboar_z",         default_value="0.109",      description="Checkerboar z"),
    DeclareLaunchArgument(name="checkerboard_qx",       default_value="0.0",        description="Checkerboar qx"),
    DeclareLaunchArgument(name="checkerboard_qy",       default_value="0.0",        description="Checkerboar qy"),
    DeclareLaunchArgument(name="checkerboard_qz",       default_value="1.0",        description="Checkerboar qz"),
    DeclareLaunchArgument(name="checkerboard_qw",       default_value="0.0",        description="Checkerboar qw"),
  ]
  return LaunchDescription(launch_args + [OpaqueFunction(function=launch_setup)])

def launch_setup(context):
  launch_moveit_path = PathJoinSubstitution([FindPackageShare('drims2_description'), 'launch', 'ur10e', 'ur10e_moveit.launch.py'])
  ...
  launch_controllers_path = PathJoinSubstitution([FindPackageShare('drims2_description'), 'launch', 'ur10e', 'ur10e_control.launch.py'])
  ...
  motion_server_path = PathJoinSubstitution([FindPackageShare("drims2_description"), "launch", "ur10e", "ur10e_motion_server.launch.py"])
  motion_server_launch = IncludeLaunchDescription(
      launch_description_source = PythonLaunchDescriptionSource(motion_server_path),
      launch_arguments = [('fake', LaunchConfiguration("fake"))]
  )

  camera_calibration_path = PathJoinSubstitution([FindPackageShare("drims2_description"), "launch", "ur10e", "ur10e_camera_calibration.launch.py"])
